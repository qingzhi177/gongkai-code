...
历史孤儿 L1 已清理（`scripts/clean_orphan_l1.py`，commit 5b04cf4）。

- **判据**：active L1 且 `conv_id` 非空且该 conv 的 active L0 = 0（来源对话整体消失）。
- **关键避坑**：`client='ai_self'` 的 AI 自述感受记忆 `conv_id=''`、本就无对应 L0，
  **不是孤儿**，判据必须排除空 conv_id（初版漏了会误删 54 条自述记忆）。
- **本次清 17 条**：全是 conv_id 修复（问题2 BugB）前被打散的 `unknown_` 对话遗留，
  软删可逆 + ChromaDB 向量已删。验证：剩余孤儿 0、ai_self 记忆 54 条完好未动。

### 8.6 目前无已知待办

需求3（修复1~8）、单条删除、L0 重复根治、问题1 孤儿清理均已完成并验证。
...
```
5b04cf4 清理脚本: 孤儿L1(问题1) + 一次性清17条
6e4f341 文档: 保留排查笔记(bug1/bug2/需求3)
1fca7d9 文档: 记录需求3收尾 + 单条删除 + L0重复根治
